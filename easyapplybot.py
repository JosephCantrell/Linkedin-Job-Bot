...
class EasyApplyBot:
   ...
   def start_apply(self, positions, locations, jump_to):
      start = time.time()
      self.fill_data()
      print(start)
      combos = []
      while len(combos) < len(positions) * len(locations):
         ...
         if combo not in combos:
            ...
            self.applications_loop(position, location, jump_to)
         if len(combos) > 500:
            break
   ...
   def applications_loop(self, position, location, jump_to):

      count_application = 0
      count_job = 0
      jobs_per_page = 0 + int(jump_to)
      ...
      while time.time() - start_time < self.MAX_SEARCH_TIME:
         ...
         for i, jobID in enumerate(jobIDs):
            ...
            if count_application != 0  and count_application % 35 == 0:
   ...
   def next_jobs_page(self, position, location, jobs_per_page):
      if args.day and args.remote:
         self.browser.get("https://www.linkedin.com/jobs/search/?f_CF=f_WRA&f_LF=f_AL&f_TPR=r86400&keywords=" + position + location + "&start="+str(jobs_per_page))
      elif args.week and args.remote:
         self.browser.get("https://www.linkedin.com/jobs/search/?f_CF=f_WRA&f_LF=f_AL&f_TPR=r604800&keywords=" + position + location + "&start="+str(jobs_per_page))
      elif args.month and args.remote:
         self.browser.get("https://www.linkedin.com/jobs/search/?f_CF=f_WRA&f_LF=f_AL&f_TPR=r2592000&keywords=" + position + location + "&start="+str(jobs_per_page))
      elif args.day:
         self.browser.get("https://www.linkedin.com/jobs/search/?f_LF=f_AL&f_TPR=r86400&keywords=" + position + location + "&start="+str(jobs_per_page))
      elif args.week:
         self.browser.get("https://www.linkedin.com/jobs/search/?f_LF=f_AL&f_TPR=r604800&keywords=" + position + location + "&start="+str(jobs_per_page))
      elif args.month:
         self.browser.get("https://www.linkedin.com/jobs/search/?f_LF=f_AL&f_TPR=r2592000&keywords=" + position + location + "&start="+str(jobs_per_page))
      else:
         self.browser.get("https://www.linkedin.com/jobs/search/?f_CF=f_WRA&f_LF=f_AL&keywords=" + position + location + "&start="+str(jobs_per_page))
      self.avoid_lock()
...
if __name__ == '__main__':
   ...
   parser.add_argument('-s', '--start', help='Change the starting page (increments of 25)', required=False, default=0)
   parser.add_argument('-r','--remote', help='Job Search using the remote filter', required=False, action='store_true',default=False)
   parser.add_argument('-d','--day', help='Job Search over the last day', required=False, action='store_true',default=False)
   parser.add_argument('-w','--week', help='Job Search over the last week', required=False, action='store_true',default=False)
   parser.add_argument('-m','--month', help='Job Search over the last month', required=False, action='store_true',default=False)
   ...
   bot.start_apply(positions, locations, args.start)
